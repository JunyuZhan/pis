...
export async function POST(request: NextRequest) {
  try {
      ...
      if (!turnstileToken) {
        ...
        try {
          ...
          if (!verifyData.success) {
            ...
            console.warn(
              '[Login] Turnstile verification failed, allowing fallback login:',
              verifyData
            );
            // 不阻止登录，继续执行登录流程
          } else {
          ...
          console.warn(
            '[Login] Turnstile verification service unavailable, allowing fallback login:',
            error
          );
          // 不阻止登录，继续执行登录流程
        }
    ...
    try {
      ...
      // 记录登录日志（异步，不阻塞响应）
      logLogin({ id: user.id, email: user.email, role: (user as { role?: string }).role }).catch(
        (err) => {
