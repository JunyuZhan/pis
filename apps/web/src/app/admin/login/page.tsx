...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (hasTurnstile && !turnstileToken && !turnstileError) {
      // 计算从页面加载到现在已经过去的时间
      const timeSincePageLoad = pageLoadTimeRef.current ? Date.now() - pageLoadTimeRef.current : 0;
      ...
      if (timeSincePageLoad > timeoutMs) {
        console.log(`[Login] Turnstile timeout (>${timeoutMs}ms), showing refresh button`);
        // 移动端显示刷新按钮，桌面端直接降级
        if (isMobile) {
        ...
        if (!turnstileToken && !turnstileError) {
          if (isMobile) {
            console.log('[Login] Turnstile not ready on mobile, showing refresh button');
            setShowRefreshButton(true);
            setLoading(false);
            return;
          } else {
            console.log('[Login] Turnstile not ready, proceeding with login (desktop fallback)');
          }
        } else {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          response.status === 428 &&
          (data.requiresPasswordSetup || data.error?.code === 'PASSWORD_NOT_SET')
        ) {
  ...
  const handleSetupPassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/auth/setup-password', {
        ...
        body: JSON.stringify({
<<<<<<< HEAD
          email: adminEmail || PIS_DEFAULT_ADMIN_EMAIL, // 使用动态获取的管理员邮箱，或回退到默认值
      ...
      try {
        ...
        console.error(
          ...
          loginErr instanceof Error ? loginErr.constructor.name : typeof loginErr
        );
        console.error(
      ...
      <div className="w-full max-w-md">
        ...
        {checkingStatus ? (
          ...
          <form onSubmit={handleSetupPassword} className="card space-y-6 p-6 sm:p-8">
            ...
            <button type="submit" disabled={setupLoading} className="btn-primary w-full">
              {setupLoading ? (
                <>
