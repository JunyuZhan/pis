...
export const userRoleSchema = z.enum(
  ...
  {
    errorMap: () => ({
      message: "角色必须是 admin、photographer、retoucher 或 guest 之一",
    }),
  },
);
...
const watermarkItemSchema = z
  .object({
    type: z.enum(["text", "logo"], {
      errorMap: () => ({ message: "水印类型必须是 text 或 logo" }),
    }),
...
export const createAlbumSchema = z
  .object({
    ...
    upload_token: z
      .string()
      .max(255, "上传令牌最多 255 个字符")
      .optional()
      .or(z.null()), // FTP/API 上传令牌（可选，创建时自动生成）
...
export const updateAlbumSchema = z
  .object({
    ...
    layout: z
      ...
      .optional(),
    sort_rule: z
      .enum(["capture_desc", "capture_asc", "manual"], {
        errorMap: () => ({ message: "无效的排序规则" }),
      })
      .optional(),
    allow_download: z.boolean().optional(),
    allow_batch_download: z.boolean().optional(),
    ...
    upload_token: z
      ...
      .max(255, "上传令牌最多 255 个字符")
      .optional()
      .or(z.null()), // FTP/API 上传令牌（可选，用于重置）
    expires_at: z.string().datetime().optional().or(z.null()),
...
export const reprocessPhotoSchema = z
  .object({
    photoIds: z
      .array(uuidSchema)
      .max(100, "单次最多重新处理100张照片")
      .optional(),
...
export const packageDownloadSchema = z
  .object({
    photoIds: z.array(uuidSchema).optional(),
    photoSelection: z
      .enum(["all", "selected", "custom"])
      .optional()
      .default("all"),
