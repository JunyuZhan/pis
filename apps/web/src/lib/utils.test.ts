...
describe('utils', () => {
  ...
  describe('formatDate', () => {
    ...
    it('should format date string', () => {
      expect(formatDate('2024-03-20')).toBe('2024年3月20日');
    });

    it('should format December correctly', () => {
      const date = new Date(2024, 11, 25); // December 25, 2024
      expect(formatDate(date)).toBe('2024年12月25日');
    });

    it('should pad single digit days', () => {
      const date = new Date(2024, 5, 5); // June 5, 2024
      expect(formatDate(date)).toBe('2024年6月5日');
    });
  });
  ...
  describe('getSafeMediaUrl', () => {
    ...
    it('should handle URLs with query parameters (returns pathname on server)', () => {
      const url = getSafeMediaUrl('https://cdn.example.com/media/photo.jpg?t=123456');
