            ...
            try {
              ...
              if (refreshResponse.ok) {
                ...
                if (refreshData.albums) {
                  ...
                  const validAlbums = refreshData.albums.filter(
                    (a: AlbumWithCover) => !a.deleted_at
                  );
      ...
      <div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
          <div>
            ...
            <p className="text-text-secondary text-sm md:text-base mt-1">管理您的所有摄影作品集</p>
          </div>
            ...
            {!selectionMode && (
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <select
                  value={shareFilter}
                  onChange={(e) =>
                    setShareFilter(e.target.value as 'all' | 'shared' | 'not_shared')
                  }
              ...
              {selectionMode ? (
                <>
                  ...
                  <button
                    ...
                    className="btn-ghost text-xs md:text-sm min-h-[44px] px-3 py-2 md:px-4 md:py-2"
                  >
                    取消
                  </button>
...
function AlbumCard({
  ...
  const coverUrl =
    album.poster_image_url && album.poster_image_url.trim()
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          <div className="space-y-1">
            <p className="text-text-secondary text-sm">{album.photo_count} 张照片</p>
            {/* 分享链接 */}
            ...
            {album.location && <p className="text-text-muted text-xs">地点：{album.location}</p>}
          </div>
        </div>
            ...
            <div className="flex items-center gap-1.5">
              <span
                className={`inline-block px-2 py-1 text-xs rounded-full ${
                  album.is_public ? 'bg-green-500/10 text-green-400' : 'bg-surface text-text-muted'
                }`}
              >
