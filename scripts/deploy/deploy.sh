...
main() {
    ...
    case $MODE in
        ...
        "need_server")
            ...
            echo ""
            echo "  1) 部署到远程服务器"
            echo "  2) 部署到当前机器（使用 docker/deploy.sh 配置向导）"
            echo "  3) 部署到当前机器（使用 scripts/deploy/deploy.sh 自动化部署）"
            echo ""
            read -p "请选择 [1-3, default: 2]: " DEPLOY_TARGET
            ...
            if [ "$DEPLOY_TARGET" = "1" ]; then
                ...
                # 使用 scripts/deploy/deploy.sh 自动化部署
                deploy_local
            fi
            ;;
        *)
