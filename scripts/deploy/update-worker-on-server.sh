...
#
# 用法:
#   bash scripts/deploy/update-worker-on-server.sh           # 完整流程（包括 git pull）
#   bash scripts/deploy/update-worker-on-server.sh --skip-pull  # 跳过 git pull（如果已手动拉取）
#   bash scripts/deploy/update-worker-on-server.sh --force   # 在本地运行
#   bash scripts/deploy/update-worker-on-server.sh --no-cache   # 使用 --no-cache 构建（不缓存层）
...
echo "     bash scripts/deploy/update-worker-on-server.sh --skip-pull"
echo ""
echo "   - 如果需要强制重新构建（不使用缓存），可以使用 --no-cache 选项:"
echo "     bash scripts/deploy/update-worker-on-server.sh --no-cache"
echo ""