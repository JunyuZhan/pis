...
# 检查是否有 label 标签（客户端渲染的页面可能在初始 HTML 中没有，这是正常的）
# 检查是否有 input 元素和相关的 id/for 属性，或者检查是否有 aria-label
test_step "表单有 label 标签或 aria-label" "echo '$login_html' | grep -qiE '<label|for=|aria-label|id=\"(username|email|password)\"' || echo '$login_html' | grep -qiE 'input.*type.*(text|email|password)'" true
