...
│  │                    Nginx (反向代理)                       │  │
│  │  • SSL/TLS 终止                                         │  │
│  │  • 路由: / → Web, /worker-api → Worker, /media → MinIO │  │
│  └───────┬───────────────────┬───────────────────┬─────────┘  │
│          │                   │                   │             │
│  ┌───────▼───────┐   ┌───────▼───────┐   ┌───────▼───────┐     │
│  │   Web         │   │   Worker      │   │   MinIO        │     │
...
### 基础设施
- **容器化**: Docker Compose
- **反向代理**: Nginx (SSL/TLS 终止，路径路由)
- **CDN**: Cloudflare (可选)
- **部署**: 完全自托管 (所有服务在 Docker 中)
...
**服务架构**:
- **前端**: Next.js 应用 (Docker 容器，端口 3000)
- **Worker**: 图片处理服务 (Docker 容器，端口 3001)
- **MinIO**: 对象存储 (Docker 容器，端口 9000/9001)
- **PostgreSQL**: 数据库 (Docker 容器，端口 5432)
- **Redis**: 任务队列和缓存 (Docker 容器，端口 6379)
- **Nginx**: 反向代理和 SSL 终止 (端口 80/443)

**域名配置**:
- **主域名**: `yourdomain.com` → Nginx (端口 443)
- **路径路由**:
  - `/` → Web 应用
  - `/worker-api/*` → Worker 服务
  - `/media/*` → MinIO 存储
  - `/minio-console/*` → MinIO 控制台（可选）

**SSL/TLS 配置**:
- 使用 Let's Encrypt 自动证书（通过 certbot）
- 或使用 Cloudflare SSL（如果使用 Cloudflare DNS）
...
Nginx (反向代理)
  ↓ HTTP POST /api/admin/albums/[id]/upload
Web API Route (Next.js)
  ↓ HTTP POST /api/worker/[...path] (代理)
Worker (Docker)
  ↓ HTTP POST (内网)
...
```
1. 上传阶段
   用户 → Nginx → Web API → Worker → MinIO (raw/)
...
#### 前端 (Docker)
- Nginx 处理 SSL/TLS 终止
- `NEXT_PUBLIC_MEDIA_URL` 使用相对路径或 HTTPS URL
